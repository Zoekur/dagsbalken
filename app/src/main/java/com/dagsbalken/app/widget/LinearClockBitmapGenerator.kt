...
object LinearClockBitmapGenerator {
    ...
    fun generate(
        ...
        for (i in events.indices) {
            ...
            // Bolt Optimization: Avoid unnecessary property access if end is null
            val actualEndMin = event.end?.let {
                val eMin = it.hour * 60 + it.minute
                if (eMin > startMin) eMin else null
            } ?: (startMin + 60)
