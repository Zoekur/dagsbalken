...
object LinearClockBitmapGenerator {
    ...
    fun generate(
        ...
        // Bolt Optimization: Use multiplication instead of division in loops.
        // Pre-calculate pixels per minute (inverse of minutes per pixel)
        val pixelsPerMinute = width.toFloat() / windowDurationMinutes.toFloat()
        ...
        for (i in events.indices) {
            ...
            // Bolt Optimization: Avoid redundant null checks/calculations for end time
            val eventEnd = event.end
            val actualEndMin = if (eventEnd != null) {
                val eMin = eventEnd.hour * 60 + eventEnd.minute
                if (eMin > startMin) eMin else startMin + 60
            } else {
                startMin + 60
            }
