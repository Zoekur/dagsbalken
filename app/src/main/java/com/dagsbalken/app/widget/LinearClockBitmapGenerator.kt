...
object LinearClockBitmapGenerator {
    ...
    fun generate(
        ...
        // Bolt Optimization: Pre-calculate pixelsPerMinute to use multiplication in loops
        // Division is significantly slower than multiplication (approx 3-10x depending on CPU).
        ...
        for (i in events.indices) {
            ...
            // Bolt Optimization: Local variable for end to avoid repeated property access
            val end = event.end
            val endMin = (end?.hour ?: 0) * 60 + (end?.minute ?: 0)
            val actualEndMin = if (end != null && endMin > startMin) endMin else startMin + 60
            ...
            if (eventStartPx + eventWidthPx > 0 && eventStartPx < width) {
                ...
                // The next operation sets color (via loop or next section), which resets alpha.
            }
        }
