...
interface TimelineProps {
  ...
  now: Date;
  /** Första synliga minut (standard 0 = midnatt) */
  dayStartMinute?: number;
  /** Sista synliga minut (standard 1440 = midnatt) */
  dayEndMinute?: number;
}
...
export default function TimelineCard({ events, customBlocks, now, dayStartMinute = 0, dayEndMinute = FULL_DAY }: TimelineProps) {
  ...
  const draw = useCallback(() => {
    ...
    const startMinute = zoom.active
      ? Math.max(dayStartMinute, zoom.centerMinute - ZOOM_WINDOW / 2)
      : dayStartMinute;
    const endMinute = zoom.active
      ? Math.min(dayEndMinute, zoom.centerMinute + ZOOM_WINDOW / 2)
      : dayEndMinute;
  ...
  }, [events, customBlocks, now, colors, dayStartMinute, dayEndMinute]);
  ...
  const handleClick = useCallback(
    (e: React.MouseEvent<HTMLCanvasElement>) => {
      ...
      const dayDuration = dayEndMinute - dayStartMinute;
      const minute = Math.round(dayStartMinute + (x / W) * dayDuration);
      zoom.active = true;
