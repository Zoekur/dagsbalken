...
class TimerRepository(private val context: Context) {
    ...
    suspend fun saveTimerTemplate(timer: TimerModel) {
        context.timerDataStore.edit { preferences ->
            ...
            // Validate primary data integrity before loading/modifying
            val isPrimaryValid = try {
            ...
            // Load with recovery logic (tries primary -> backup -> empty)
            ...
            // Only update backup if the primary source was valid.
            // If primary was corrupted, we keep the existing backup (which we likely just used for recovery).
            if (isPrimaryValid) {
                preferences[TIMER_TEMPLATES_BACKUP_KEY] = jsonString
            } else {
                Log.w(TAG, "Primary storage corrupted, skipping backup update during save")
            }
    ...
    suspend fun deleteTimerTemplate(timerId: String) {
        context.timerDataStore.edit { preferences ->
            ...
            if (isPrimaryValid) {
                ...
                Log.w(TAG, "Primary storage corrupted, skipping backup update during delete")
            }
    ...
    suspend fun addActiveTimer(block: CustomBlock) {
        context.timerDataStore.edit { preferences ->
            ...
            if (isPrimaryValid) {
                ...
                Log.w(TAG, "Primary active timers corrupted, skipping backup update")
            }
