...
class TimerRepository(private val context: Context) {
    ...
    private val logWarning: (String, Throwable?) -> Unit = { msg, err ->
    ...
    val timerTemplatesFlow: Flow<List<TimerModel>> = context.timerDataStore.data
        .map { preferences ->
            ...
            TimerSerializer.deserializeTimerTemplatesSafe(jsonString, backupString, logWarning)
        }

    suspend fun saveTimerTemplate(timer: TimerModel) {
        context.timerDataStore.edit { preferences ->
            val jsonString = preferences[TIMER_TEMPLATES_KEY] ?: "[]"
            // Use STRICT deserialization for writes to prevent data loss!
            // If this throws, the transaction aborts, and we don't overwrite with an empty list.
            val initialList = TimerSerializer.deserializeTimerTemplatesStrict(jsonString)
    ...
    val activeTimersFlow: Flow<List<CustomBlock>> = context.timerDataStore.data
        .map { preferences ->
            ...
            TimerSerializer.deserializeActiveTimersSafe(jsonString, backupString, logWarning)
        }
    ...
    suspend fun addActiveTimer(block: CustomBlock) {
        context.timerDataStore.edit { preferences ->
            val jsonString = preferences[ACTIVE_TIMERS_KEY] ?: "[]"
            // Use STRICT deserialization for writes to prevent data loss!
            val initialList = TimerSerializer.deserializeActiveTimersStrict(jsonString)
            val currentList = initialList.toMutableList()
