...
class TimerRepository(private val context: Context) {
    ...
    private fun deserializeTimerTemplatesWithRecovery(jsonString: String, backupString: String?, throwOnFailure: Boolean = false): List<TimerModel> {
        return try {
            ...
            if (throwOnFailure) {
                throw IOException("Failed to recover timer templates from both primary storage and backup", e)
            }
            emptyList()
        }
    }
    ...
    private fun deserializeActiveTimersWithRecovery(jsonString: String, backupString: String?, throwOnFailure: Boolean = false): List<CustomBlock> {
        return try {
            ...
            if (throwOnFailure) {
                throw IOException("Failed to recover active timers from both primary storage and backup", e)
            }
            emptyList()
        }
    }
