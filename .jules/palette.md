...
## 2025-12-13 - Standardizing Dropdowns
**Learning:** Custom `Row` + `Text` dropdowns lack visual affordance and accessibility traits (like proper labeling).
**Action:** Use `ExposedDropdownMenuBox` with a read-only `OutlinedTextField` and `MenuAnchorType.PrimaryNotEditable` for all selection inputs to ensure consistency and accessibility.
